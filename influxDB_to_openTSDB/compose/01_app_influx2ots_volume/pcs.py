        ...
        for _field in meta['field']:
            ...
            for i in range(len(dftime)):
                ...
                if value == 'nan' or dftime[i] == 'nan':
                    ...
                    continue
                ts = influx2tsdb.convertRFV3339ToEpoch(dftime[i])
                ts = str(ts)
                _dict = dict()
...
def _consume(works_to_do, _empty1, meta, _empty2):
    ...
    while True:
        ...
        if works == 'END':
            ...
            try:
                response = sess.post(meta['ots_url'], data=json.dumps(works), headers=headers)
                ...
                while (response.status_code > 204):
                    ...
                    response = sess.post(meta['ots_url'], data=json.dumps(works), headers=headers)  # , timeout=10)
                    tries += 1
